...
for i in search(input, tld="co.in", num=50):# num=30, stop=30, pause=2):
    print("starting loop")
    try:
        #webscraping
        url = i
        ...
        f = open(r"process\cross_ref.txt", "w", encoding="utf-8")
        f.write(text)
        print("process finished")

        #compare semantic similarity between website and original input
        file1 = open(r"process\cross_ref.txt", encoding="utf-8").read()
        doc1 = nlp(file1)
        file2 = open(r"process\user_text.txt", encoding="utf-8").read()
        doc2 = nlp(file2)
        a = doc1.similarity(doc2)
        print(a)
        f = open(r"process\tempstore.txt", "a", encoding="utf-8")
        f.write("\n")
        f.write(str(a))
